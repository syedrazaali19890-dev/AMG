...
export default function ScalpingV2Page() {
    ...
    const filteredRunningSignals = runningSignals.filter(s => {
        if (runningSubFilter === 'ACTIVE') return s.status === 'ACTIVE';
    ...
    const generateSignals = useCallback(async () => {
        ...
        try {
            ...
            setRunningSignals(prev => {
                const activeFromCurrent = signals.filter(s => s.status === 'ACTIVE');
                    ...
                    {selectedMarket && (
                        <div>
                            ...
                            <div className="flex border-b border-white/5 mb-6">
                                ...
                                >
                                    Running Trades ({runningSignals.filter(s => s.status === 'ACTIVE').length})
                                    {runningSignals.filter(s => s.status === 'ACTIVE').length > 0 && (
                            ...
                            {activeTab === 'RUNNING' && (
                                <div className="flex gap-2 mb-6 bg-white/[0.02] p-1.5 rounded-xl border border-white/5 w-fit">
                                    {[
                                        { key: 'ACTIVE', label: 'Active', count: runningSignals.filter(s => s.status === 'ACTIVE').length, color: 'text-cyan-400' },
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                                {activeTab === 'NEW' ? (
                                    ...
                                    [
                                        ...
                                        {
                                            label: 'Active Trades',
                                            value: runningSignals.filter(s => s.status === 'ACTIVE').length,
                                            icon: Activity,
                                            color: 'text-cyan-400',
                                            bg: 'bg-cyan-500/10'
                                        },
